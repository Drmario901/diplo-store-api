...
if (!isset($input['token'], $input['user_id'], $input['new_role_id'])) {
    http_response_code(400);
    echo json_encode(["message" => "Missing parameters: token, user_id or new_role_id"]);
    exit();
}

$token = $input['token'];
$userId = intval($input['user_id']);
$newRoleId = intval($input['new_role_id']);
...
try {
    ...
    $update = mysqli_query($conexion, "UPDATE users SET role_id = $newRoleId WHERE id = $userId");
